 ...
 * {@code CreateAcademicPlanUseCase.AcademicPlanResult}). {@code levels} is
 * empty except when returned by {@code GET /plans/{id}} (design.md —
 * "GET /plans/{id} is the only endpoint returning the full nested tree").
 */
public record AcademicPlanResponse(UUID id, UUID programId, String version, String validityPeriod,
		...
		PlanStatus status, List<PlanLevelResponse> levels) {
}
